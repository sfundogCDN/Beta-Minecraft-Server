...
//            EntityHuman, ChunkCoordinates, EnumBedError, Direction, 
//            Item, IBlockAccess
...
public class BlockBed extends Block
{
    ...
    public boolean interact(World world, int i, int j, int k, EntityHuman entityhuman)
    {
        ...
        if(!world.worldProvider.d())
        {
            ...
            if(world.getTypeId(i, j, k) == id)
            {
                ...
                d3 = (d3 + (double)k + 0.5D) / 2D;
            }
            world.createExplosion(null, (float)i + 0.5F, (float)j + 0.5F, (float)k + 0.5F, 5F, true);
            return true;
        }
        if(e(l))
        {
    ...
    public void doPhysics(World world, int i, int j, int k, int l)
    {
        ...
        if(d(i1))
        {
            if(world.getTypeId(i - a[j1][0], j, k - a[j1][1]) != id)
                world.setTypeId(i, j, k, 0);
        } else
        if(world.getTypeId(i + a[j1][0], j, k + a[j1][1]) != id)
        {
